...
def test_ocr_gate_caches_after_lock():
    ...
    tracker = GatedOcrTracker(ocr_fn=ocr_fn, gate_confidence=0.95)
    frame = _box_frame()
    boxes = np.array([[10, 10, 50, 50]])
    scores = np.array([0.9])       # above track_high_thresh -> track on frame 1
    classes = np.array([3])
...
def test_ocr_fn_exception_does_not_crash_update():
    ...
    tracker = GatedOcrTracker(ocr_fn=ocr_fn)
    frame = _box_frame()
    boxes = np.array([[10, 10, 50, 50]])
    scores = np.array([0.9])
    classes = np.array([2])

    tracks = tracker.update(boxes, scores, classes, frame)  # must not raise
    assert tracks[0].locked is False
...
def test_track_loss_is_logged_not_crashed(caplog):
    ...
    # track_buffer=0 -> a lost track is dropped immediately (was max_age=0).
    tracker = GatedOcrTracker(ocr_fn=ocr_fn, overrides={"track_buffer": 0})
    frame = _box_frame()
    scores = np.array([0.9])
    classes = np.array([2])

    tracker.update(np.array([[10, 10, 50, 50]]), scores, classes, frame)
    # No matching detection next frame -> track should age out without crashing.
    tracks = tracker.update(
        np.empty((0, 4)), np.empty((0,)), np.empty((0,)), frame
    )
    assert tracks == []
